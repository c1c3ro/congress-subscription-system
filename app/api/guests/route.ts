...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase.from("test_guests").select("*").order("created_at", { ascending: true })
...
export async function DELETE(request: Request) {
  try {
    ...
    const { error: confirmError } = await supabase.from("test_confirmations").delete().eq("guest_id", id)

    if (confirmError) {
      console.error("Erro ao deletar confirmação:", confirmError)
    }

    const { error: guestError } = await supabase.from("test_guests").delete().eq("id", id)
