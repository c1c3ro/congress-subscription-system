...
-- Atribuir números aos inscritos já confirmados (por ordem de created_at)
-- UTI: 1 a 200
WITH uti_numerados AS (
  SELECT id, ROW_NUMBER() OVER (ORDER BY created_at) as num
  FROM inscricoes
  WHERE congresso = 'uti' AND confirmacao_finalizada = true AND numero_sorteio IS NULL
)
UPDATE inscricoes
...
WITH pedneo_numerados AS (
  ...
  FROM inscricoes
  WHERE congresso = 'utipedneo' AND confirmacao_finalizada = true AND numero_sorteio IS NULL
)
UPDATE inscricoes
